...
// Calls RenderBuffers to put the sky plane geometry on the graphics pipeline for drawing.
...
// Returns the number of indexes in the index buffer.
int SkyPlaneClass::GetIndexCount()
{
	return m_indexCount;
}

// Returns the cloud texture resource.
ID3D11ShaderResourceView* SkyPlaneClass::GetCloudTexture()
{
	return m_CloudTexture->GetTexture();
}

// Returns the perturb texture resource.
...
// Returns the current brightness value that will be applied to the clouds in the pixel shader.
...
// Builds the geometry for the sky plane.
...
// Loads the array that was holding the sky plane geometry into a vertex and index buffer so that it can be rendered.
...
// Puts the sky plane geometry on the graphics pipeline for rendering by the shader.
...
// Loads the two cloud textures that will be used for rendering with.
bool SkyPlaneClass::LoadTextures(ID3D11Device* device, WCHAR* cloudTextureFilename, WCHAR* perturbTextureFilename)
{
	bool result;
	...
	result = m_CloudTexture->Initialize(device, cloudTextureFilename);
	if (!result)
	{
		return false;
	}
